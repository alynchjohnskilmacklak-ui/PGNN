...
def isa_pressure(alt_m: float, P_sea: float = 101325.0) -> float:
    """ISA 标准大气下海拔 alt_m 处的压力（Pa）。用于让测试集物理自洽。"""
...
def summarize_and_save(df: pd.DataFrame, out_dir: str, has_nn: bool):
    ...
    method_names = {
        ...
        "C": "C: NN-init + Grid+Newton (existing)",
    ...
    if has_nn:
        ...
        # ========= 主对照：C vs E（设计原意验证）=========
        print("  >>> Primary: 你的设计原意 (NN -> Newton, no grid) vs 现状 (NN -> Grid+Newton)")
        ...
        if e_med < a_med * 0.5 and a_med > 0.05:
            print("      ✓ NN 初值确实比真空初值精度高 ——> NN 训练有价值。")
        elif abs(e_med - a_med) / max(a_med, 1e-9) < 0.3:
            print("      ⚠ NN 初值精度跟真空初值接近 ——> 牛顿在做大部分工作，NN 是装饰。")
