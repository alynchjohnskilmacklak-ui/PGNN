"""Differentiable ballistics consistency loss functions for neural network training."""
...
class SmoothPhysicsLoss(nn.Module):
    """Short-horizon differentiable trajectory consistency regularizer.

    This loss compares predicted and labeled angles under the same sampled
    environment. It is not a full target-intercept residual at x_target.
    """
    ...
    def __init__(
        ...
        if self.loss_mode not in ("consistency", "target_intercept"):
            raise ValueError("loss_mode must be 'consistency' or 'target_intercept'.")
    ...
    def forward(self, pred_angles: torch.Tensor, X_raw: torch.Tensor):
        return self.integrate_fixed_steps(pred_angles, X_raw, dt=self.dt, steps=self.steps)
    ...
    def integrate_fixed_steps(
        ...
        if not torch.is_tensor(dt):
            dt = torch.full_like(vx, float(dt))
        else:
            dt = dt.to(device=vx.device, dtype=vx.dtype)
    ...
    def target_intercept_loss(self, pred_angles: torch.Tensor, X_raw: torch.Tensor) -> torch.Tensor:
        x_target = torch.clamp(X_raw[:, 0:1], min=1.0)
        y_target = X_raw[:, 1:2]
        z_target = X_raw[:, 2:3]

        theta_rad = pred_angles[:, 0:1] * (math.pi / 180.0)
        alpha_rad = pred_angles[:, 1:2] * (math.pi / 180.0)
        v0 = torch.clamp(X_raw[:, 3:4], min=1.0)
        vx0 = torch.clamp(v0 * torch.cos(theta_rad) * torch.cos(alpha_rad), min=1.0)
        t_est = torch.clamp(x_target / vx0, min=0.01, max=30.0)
        dt = t_est / float(self.steps)

        _, pred_y, pred_z = self.integrate_fixed_steps(pred_angles, X_raw, dt=dt, steps=self.steps)
        sy = torch.clamp(torch.abs(y_target), min=1.0)
        sz = torch.clamp(torch.abs(z_target), min=1.0)
        ly = _per_elem_smooth_l1((pred_y - y_target) / sy, delta=0.05)
        lz = _per_elem_smooth_l1((pred_z - z_target) / sz, delta=0.05)
        return ly + lz
    ...
    def regularization_loss(
        ...
        if self.loss_mode == "target_intercept":
            return self.target_intercept_loss(pred_angles, X_raw)
        return _physics_consistency_loss(self, pred_angles, y_true, X_raw)
...
_physics_residual_loss = _physics_consistency_loss
...
def _forward_loss_dict(
    ...
    if lambda_eff > 0.0:
        loss_phys = _weighted_mean(phys_engine.regularization_loss(pred, yb, X_raw), sample_w)
    else:
