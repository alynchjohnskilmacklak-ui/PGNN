...
def _process_single_angle_worker(args):
    ...
    for alt_m in alts:
        ...
        P0_Pa = float(rng.uniform(P0_Pa_range[0], P0_Pa_range[1]))

        # 药温解耦逻辑：70% 受环境影响，30% 独立
        if rng.random() < 0.7:
            T_powder = T0_C + float(rng.normal(0, 3.0))
        else:
            T_powder = float(rng.uniform(-10.0, 60.0))
...
def generate_dataset(
    ...
    trajectories, ranges, alt_bins_used = [], [], alt_bins
    ...
    for traj in trajectories:
        ...
        is_high_angle = 1.0 if th >= 50.0 else 0.0
    ...
    cols = [
        ...
        "is_high_angle", "T0_C", "P0_Pa", "alt_gun",
        "alpha_deg",
        "theta_deg"
    ]
